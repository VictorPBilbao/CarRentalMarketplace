<script lang="ts">
  ...
  $effect(() => {
    if ((form as any)?.addonSucesso || (form as any)?.feeSucesso) {
      modalAddon = false;
      modalFee = false;
    }
  });
...
<div style="max-width:1100px;">
  ...
  {#if abaAtiva === 'rateplans'}
    <div style="background:rgba(251,191,36,0.06); border:1px solid rgba(251,191,36,0.2); border-radius:10px; padding:12px 16px; margin-bottom:20px; font-size:13px; color:#94a3b8; display:flex; gap:10px; align-items:flex-start;">
      <svg width="15" height="15" viewBox="0 0 15 15" fill="none" style="flex-shrink:0; color:#fbbf24; margin-top:1px">
        <path d="M7.5 1.5L13.5 13H1.5L7.5 1.5z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
        <path d="M7.5 6v3.5M7.5 11v.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
      </svg>
      <span><strong style="color:#fbbf24;">Rate Plans</strong> são somente leitura nesta versão. O CRUD de planos requer endpoints adicionais (<code style="background:rgba(255,255,255,0.06); border-radius:3px; padding:1px 5px;">POST/PUT /locadora/rate_plans</code>) a serem implementados.</span>
    </div>
  {/if}

<script lang="ts">
  ...
  const TRANSICOES: Record<string, Array<{ status: string; label: string; cor: string; corHover: string }>> = {
    PENDING:   [
      { status: 'CONFIRMED', label: 'Confirmar',       cor: '#60a5fa', corHover: '#3b82f6' },
      { status: 'CANCELLED', label: 'Cancelar',        cor: '#f87171', corHover: '#ef4444' },
    ],
    CONFIRMED: [
      { status: 'ACTIVE',    label: 'Marcar Retirada', cor: '#4ade80', corHover: '#22c55e' },
      { status: 'NO_SHOW',   label: 'Não Compareceu',  cor: '#fb923c', corHover: '#f97316' },
      { status: 'CANCELLED', label: 'Cancelar',        cor: '#f87171', corHover: '#ef4444' },
    ],
    ACTIVE: [
      { status: 'COMPLETED', label: 'Concluir',        cor: '#94a3b8', corHover: '#64748b' },
      { status: 'CANCELLED', label: 'Cancelar',        cor: '#f87171', corHover: '#ef4444' },
    ],
    COMPLETED: [],
    CANCELLED: [],
...
<svelte:head>
  <title>Reserva {shortId(data.reserva.id)} — Locadora</title>
</svelte:head>
      ...
      {#each transicoes as t}
        <form method="POST" action="?/atualizarStatus">
          ...
          <button
            type="submit"
            class="btn-acao"
            style="--cor: {t.cor}; --cor-hover: {t.corHover};"
          >{t.label}</button>
        </form>
      {/each}
    </div>
      ...
      <dl class="info-list">
        ...
        <div class="info-item">
          ...
          <dd>
            ...
            <span class="loja-tag">{nomeLoja(data.reserva.pickup_store)}</span>
          </dd>
        </div>
        <div class="info-item">
          <dt>Devolução</dt>
          <dd>
            ...
            <span class="loja-tag">{nomeLoja(data.reserva.dropoff_store)}</span>
          </dd>
        </div>
...
<style>
  ...
  .total-val { font-size: 18px; font-weight: 700; color: #60a5fa !important; }

  .banner-sucesso {
    display: flex; align-items: center; gap: 10px;
    margin-bottom: 20px; padding: 12px 16px; border-radius: 10px;
    border: 1px solid rgba(96,165,250,0.2); background: rgba(96,165,250,0.07);

...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const [veiculo, categorias] = await Promise.all([
    filialVeiculoService.buscarPorId(id, token).catch(() => null),
    categoriaService.listar(token).catch(() => []),
  ]);

  if (!veiculo) error(404, 'Veículo não encontrado ou não pertence a esta loja.');

  return { veiculo, categorias };
};
...
export const actions: Actions = {
  atualizar: async ({ params, request, locals, cookies }) => {
    ...
    try {
      await filialVeiculoService.atualizar(
        id,

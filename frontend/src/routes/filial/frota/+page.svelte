...
<div class="page-header">
  <div>
    <h1>Frota da Loja</h1>
    <p>Veículos disponíveis e sob responsabilidade desta filial</p>
  </div>
  <a href="/filial/frota/novo" class="btn-novo">
...
<div class="stats-row">
  <div class="stat-card">
    <p class="stat-label">Total na Loja</p>
...
{#if data.veiculos.length === 0}
  <div class="empty-state">
    ...
    <p class="empty-titulo">Nenhum veículo nesta loja</p>
    <p class="empty-desc">Cadastre o primeiro veículo da sua frota.</p>
    <a href="/filial/frota/novo" class="btn-novo" style="margin-top:16px">Cadastrar Veículo</a>
  </div>
{:else}
      ...
      <tbody>
        {#each data.veiculos as v (v.id)}
          ...
          <tr>
            ...
            <td><span class="placa-badge">{v.plate.toUpperCase()}</span></td>
            <td>
              {#if cat}
                <div class="cat-info">
                  <span style="font-size:13px; color:#cbd5e1">{cat.group_name}</span>
                  <span style="font-size:11px; color:#334155">{cat.code}</span>
                </div>
              {:else}
                <span style="color:#334155">—</span>
              {/if}
            </td>
            <td><span class="km-val">{v.mileage_km.toLocaleString('pt-BR')} km</span></td>
            <td>
            ...
            <td class="td-acoes">
              <a href="/filial/frota/{encodeURIComponent(v.id)}/editar" class="btn-editar">
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
...
<style>
  ...
  .btn-editar:hover { border-color: rgba(52,211,153,0.4); color: #34d399; }

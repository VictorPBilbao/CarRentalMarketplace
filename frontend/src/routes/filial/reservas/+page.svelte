...
<div class="page-header">
  <div>
    <h1>Reservas da Loja</h1>
    <p>Reservas com retirada em {filial?.name ?? 'esta loja'}</p>
  </div>
</div>
...
<div class="filtros-wrap">
  {#each FILTROS as f}
    <a
      href={f.valor ? `/filial/reservas?status=${f.valor}` : '/filial/reservas'}
      class="filtro-btn"
...
{#if data.reservas.length === 0}
  <div class="empty-state">
    ...
    <p class="empty-desc">
      {data.statusFiltro ? 'Nenhuma reserva com este status.' : 'Ainda não há reservas para esta loja.'}
    </p>
  </div>
{:else}
          ...
          <tr>
            ...
            <td>
              <div class="periodo-cell">
                ...
                <span class="periodo-sep">até {formatDate(r.dropoff_time)}</span>
              </div>
            </td>
            <td>
            ...
            <td class="td-acoes">
              <a href="/filial/reservas/{encodeURIComponent(r.id)}" class="btn-ver">Ver</a>
            </td>
          </tr>
        {/each}
      </tbody>
...
<style>
  ...
  .cliente-id { font-family: monospace; font-size: 11px; color: #64748b; }
  .periodo-cell { display: flex; flex-direction: column; gap: 2px; font-size: 12px; color: #94a3b8; }
  .periodo-sep  { font-size: 11px; color: #475569; }
  ...
  .btn-ver:hover { border-color: rgba(52,211,153,0.4); color: #34d399; }

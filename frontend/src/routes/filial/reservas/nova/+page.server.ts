...
export const actions: Actions = {
  cotar: async ({ request, locals }) => {
    ...
    const campos = { customerId, categoryId, dropoffStoreId, pickupTime, dropoffTime,
                     customerAge: String(customerAge), nationality: nationality ?? '',
                     promoCode: promoCode ?? '', flightNumber: flightNumber ?? '', notes: notes ?? '' };
  ...
  confirmar: async ({ request, locals, cookies }) => {
    ...
    let cotacao;
    try {
      cotacao = await tarifaService.calcularCotacaoFilial(
        {
          pickup_store_id:  '',
          dropoff_store_id: dropoffStoreId,
          category_id:      categoryId,

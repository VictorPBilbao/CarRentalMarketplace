...
async def listar_fees_empresa(company_id: str, db: AsyncSurreal) -> list[dict]:
    result = await db.query(
        """
...
async def listar_addons_empresa(company_id: str, db: AsyncSurreal) -> list[dict]:
    result = await db.query(
        """
        SELECT * FROM addon
        WHERE company = type::record($cid)
...
async def calcular_cotacao(
    ...
    if payload.rate_plan_id:
        ...
        if not plan:
            raise HTTPException(
                status_code=422,
                detail='O plano tarifário informado não é aplicável para estes parâmetros.',
            )
    else:
        plan = plans_response[0]  # já ordenado por menor diária
    ...
    for sel in payload.selected_addons:
        ...
        breakdown.append(ItemCotacao(
            type='ADDON',
            description=f'{addon_resp.name} × {sel.quantity}',
            amount=calculated,
        ))

    # Taxa de retorno one-way
    ...
    if is_one_way and one_way_raw and isinstance(one_way_raw, dict):
        ...
        if fee_type == 'FIXED':
            ...
            one_way_fee_amount = 0.0
        # PER_KM: requer distância — incluído como 0 (frontend deve exibir aviso)
    ...
    # Proteções incluídas no plano
    protecoes = await _buscar_protecoes(
        company_id, plan.included_protections, payload.category_id, db
    )
